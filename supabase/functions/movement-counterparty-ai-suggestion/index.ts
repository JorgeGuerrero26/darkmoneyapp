/**
 * Deploy:
 *   npx supabase functions deploy movement-counterparty-ai-suggestion --project-ref <project-ref>
 *
 * Required secrets:
 *   SUPABASE_URL
...
type CounterpartyType = "person" | "company" | "merchant" | "service" | "bank" | "other";
...
function sanitizeCounterpartyType(value: unknown): CounterpartyType {
  return typeof value === "string" && COUNTERPARTY_TYPES.has(value as CounterpartyType)
...
function sanitizeCounterparties(value: unknown): CounterpartyInput[] {
  if (!Array.isArray(value)) return [];
  const seen = new Set<number>();
  const counterparties: CounterpartyInput[] = [];
  for (const item of value) {
    ...
    counterparties.push({
      ...
      type: sanitizeCounterpartyType(row.type),
    });
    if (counterparties.length >= 80) break;
  }
...
function normalizeRecommendation(raw: string, counterparties: CounterpartyInput[]): Recommendation {
  ...
  try {
    ...
    const type = parsed.type === "existing_counterparty" || parsed.type === "new_counterparty" ? parsed.type : "none";
    ...
    const counterpartyType = sanitizeCounterpartyType(parsed.counterpartyType);

    if (type === "existing_counterparty") {
      const counterpartyId = numberFromBody(parsed.counterpartyId);
      const counterparty = counterpartyId ? counterparties.find((item) => item.id === counterpartyId) : null;
      if (!counterparty) return fallback;
      return {
        type,
        ...
        confidence,
        reasons: reasons.length ? reasons : ["coincide con una contraparte existente"],
      };
    }

    if (type === "new_counterparty") {
      const rawName = typeof parsed.newCounterpartyName === "string" ? parsed.newCounterpartyName : "";
      const newCounterpartyName = rawName.trim().replace(/\s+/g, " ").slice(0, 64);
      if (newCounterpartyName.length < 3) return fallback;
      const duplicate = counterparties.find((item) => normalizeText(item.name) === normalizeText(newCounterpartyName));
      if (duplicate) {
        return {
          type: "existing_counterparty",
          counterpartyId: duplicate.id,
          counterpartyName: duplicate.name,
          newCounterpartyName: null,
          counterpartyType: duplicate.type,
          confidence,
          reasons: reasons.length ? reasons : ["la contraparte ya existe con un nombre equivalente"],
        };
      }
      return {
        type,
        ...
        counterpartyType,
        confidence,
        reasons: reasons.length ? reasons : ["no hay una contraparte existente suficientemente clara"],
      };
    }
...
function buildPrompt(input: {
  ...
  return [
    "Eres el asistente financiero de DarkMoney para sugerir contrapartes.",
    "Una contraparte es una persona, comercio, empresa, banco o servicio asociado al movimiento.",
    "Debes elegir una contraparte existente o proponer crear una nueva si el nombre se puede inferir claramente.",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const counterparties = sanitizeCounterparties(body.counterparties);
    ...
    console.error("[movement-counterparty-ai-suggestion]", error);
    const message = error instanceof Error ? error.message : "No se pudo sugerir la contraparte.";
