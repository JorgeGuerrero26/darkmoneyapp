/**
 * Deploy:
 *   npx supabase functions deploy movement-recurring-ai-suggestion --project-ref <project-ref>
 *
 * Required secrets:
 *   SUPABASE_URL
...
function compactArray(value: unknown, max: number) {
...
function normalizeRecommendation(raw: string, movementType: MovementType): Recommendation {
  ...
  try {
    ...
    const requestedType = parsed.type === "subscription" || parsed.type === "recurring_income" ? parsed.type : "none";
    const type = movementType === "income"
      ? requestedType === "recurring_income" ? requestedType : "none"
      : requestedType === "subscription" ? requestedType : "none";
    if (type === "none") return fallback;
    const rawName = trimText(parsed.name, 64);
    const frequency = typeof parsed.frequency === "string" && FREQUENCIES.has(parsed.frequency as Frequency)
      ? parsed.frequency as Frequency
      : null;
    const rawInterval = Number(parsed.intervalCount ?? 1);
    const intervalCount = Number.isFinite(rawInterval) && rawInterval > 0 ? Math.min(12, Math.round(rawInterval)) : 1;
    ...
    if (!rawName || !frequency) return fallback;
    return {
      type,
      name: rawName,
      frequency,
      intervalCount,
      confidence,
      reasons: reasons.length ? reasons : ["parece repetirse con una cadencia estable"],
    };
  } catch {
    return fallback;
  }
...
function buildPrompt(input: {
  ...
  return [
    "Eres el asistente financiero de DarkMoney para detectar pagos o ingresos recurrentes.",
    "Si el movimiento es expense, solo puedes recomendar subscription o none.",
    "Si el movimiento es income, solo puedes recomendar recurring_income o none.",
    "No recomiendes si ya existe una suscripcion o ingreso fijo muy parecido en las listas enviadas.",
    "Usa datos minimos: monto, descripcion, contraparte, categoria y movimientos recientes.",
    "No menciones IA, DeepSeek ni el proveedor.",
    "Devuelve solo JSON valido con esta forma exacta:",
    '{"type":"subscription|recurring_income|none","name":"nombre"|null,"frequency":"weekly|biweekly|monthly|quarterly|yearly"|null,"intervalCount":1,"confidence":0.0,"reasons":["razon breve"]}',
    "",
    "Datos:",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const description = trimText(body.description, 180);
    ...
    const prompt = buildPrompt({
      ...
      recentMovements: compactArray(body.recentMovements, 30),
      subscriptions: compactArray(body.subscriptions, 40),
      recurringIncome: compactArray(body.recurringIncome, 40),
      localSuggestion: body.localSuggestion ?? null,
    });
    const rawReply = await requestDeepSeek(apiKey, model, prompt);
    const recommendation = normalizeRecommendation(rawReply, movementType);
    await recordUsage({
      client,
      userId: user.id,
      workspaceId,
    ...
    console.error("[movement-recurring-ai-suggestion]", error);
    const message = error instanceof Error ? error.message : "No se pudo detectar recurrentes.";
